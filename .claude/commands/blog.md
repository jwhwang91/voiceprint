...
### 0. 기존 초안 확인 (⭐ 항상 먼저 — 사진 재분석 낭비 방지)
글쓰기/사진분석을 **시작하기 전에**, `data/drafts/<job>/post.md` 와 `data/drafts/<job>/layout.json` 이
이미 있는지 확인한다(`ls data/drafts/<job>/`).
- **둘 다 있으면**: 사진을 다시 분석하지 말고, 기존 `post.md` 제목·문단 수·사진 수를 1줄로 요약해서
  **"이미 작성된 초안이 있어요 — 이대로 발행할까요? (예: 바로 발행 / 아니오: 새로 작성)"** 라고 묻는다.
  - **예/발행** → 사진 태깅·SEO·본문 작성을 **모두 건너뛰고** 곧장 **아래 9번(발행)** 으로 간다.
  - **아니오/새로** → 기존 초안을 새로 덮어쓰며 1번부터 정상 진행한다.
- **없으면**: 묻지 말고 1번부터 정상 진행한다.
...
9. `python main.py publish --job <job명> --yes` 를 실행한다. ⚠️ **`--yes` 필수** — 너는 Bash 로 실행하므로 stdin 이 없어 확인 프롬프트에서 "비대화형 환경… 중단" 으로 죽는다(8번에서 이미 사용자에게 발행 확인을 받았으니 `--yes` 로 자동 진행).
...
11. **실패 시 (DOM/셀렉터 오류) — 추측 말고 살아있는 DOM 을 보고 고친다(최대 3회 시도)**:
    ...
    - `python main.py publish --job <job명> --yes` 재시도. 또 다른 셀렉터에서 실패하면 이 치유 루프를 반복하되 **발행 시도는 총 3회까지만**.
...
### 발행 (자가 치유 포함)
7. `python main.py publish --engage <job명> --yes` 를 실행한다(`--yes` 필수 — 비대화형 중단 방지).
